...
def displayhtml(site_key, language, theme, fallback=False, d_type='image'):
    """
    ...
    return """<script src="https://www.google.com/recaptcha/api.js?hl=%(LanguageCode)s&fallback=%(Fallback)s" async defer></script>
              <div class="g-recaptcha" data-sitekey="%(SiteKey)s" data-theme="%(Theme)s" data-type="%(Type)s"></div>
              <noscript>
                    <div style="width: 302px; height: 352px;">
                        <div style="width: 302px; height: 352px; position: relative;">
                              <div style="width: 302px; height: 352px; position: absolute;">
                                    <iframe src="https://www.google.com/recaptcha/api/fallback?k=%(SiteKey)s&hl=%(LanguageCode)s"
...
def submit(recaptcha_response_field, secret_key, remoteip, verify_server=VERIFY_SERVER):
    ...
    if use_requests:
        httpresp = requests.get(url="https://%s/recaptcha/api/siteverify" % verify_server,
                                data=params,
                                headers={
